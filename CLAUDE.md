...
**An org dependency is pinned in `go.mod`, and CI builds that pin.** A CI build is `GITHUB_ACTIONS=true` with no coding agent in the environment or among the ancestor processes. It looks nothing up, so a commit that lands mid-run reaches no job of it. A placeholder there is an error. Every run outside CI moves the pin to the branch head and writes it silently, with the third-party requirements the org module declares. Which head, and the rest: docs/ORG-DEPS.md. A gitlink never pins an org submodule. The cache client's frozen gitlink is what broke `cmd/go`: `shared.go` named counters that commit did not carry, and no toolchain built. Every `wow-look-at-my` submodule follows a branch. `submodulebranch.bash` names that branch, then runs `git submodule update --init --remote`. make.bash, make.bat and CI all call it, and `branch = master` in `.gitmodules` is the fallback. A third-party dependency keeps its version.
