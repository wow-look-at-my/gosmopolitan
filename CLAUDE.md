...
**The cache stores bytes under a key of source and compiler. Nothing else.** `PutExecutable` writes `<outputID>-d` as a directory holding one file named for the binary. That name is what lets the build run it: mode 0777, plus the `.exe` Windows needs before exec accepts the path. It offers the shared tier the same plain bytes as any other object, so the NAME never goes on the wire. A restore therefore writes the plain file `Put` writes. `cache.GetExecutableFile` rewrites it as that directory the first time the build asks to run it. It takes the name from the caller (`Internal.ExeName`, else `DefaultExecName`, plus `cfg.ExeSuffix`), because `useCache` has the package in hand. Skipping that call is `fork/exec ...-d: permission denied`.
