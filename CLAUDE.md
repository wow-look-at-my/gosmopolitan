...
- **The go command is the toolchain.** `bin/go` links every tool and starts each as `<self> tool <name>`. Every `pkg/tool` entry links to it. The main is `cmd/go/main`; `cmd/go` is `package gocmd`; each tool is a package with `Main(args []string) int`. The cosmo `os/exec` runs an `ENOEXEC` `MZqFpD` file under `/bin/sh`, so tests need no wrapper.
- **A go binary can carry its standard library.** `go tool embedstd -o blob` writes it; `GOCOSMOAPPEND=blob` appends it to the APE. With no `GOROOT` set, that go command builds against it and refuses to test or vet std. Depth: docs/EMBEDDED-STD.md.
