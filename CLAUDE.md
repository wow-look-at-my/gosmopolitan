...
# is still stripped and given its sidecar. The APE header is a fixed 64K, so
# only dropping an ARCHITECTURE changes the size (-47% for amd64-only) --
# linux/amd64+darwin/arm64+windows/amd64 still needs both payloads and
# weighs exactly what the fat APE does. `go env GOCOSMOPLATFORMS` reports
