...
- **A direct fetch from github.com downloads an archive before it runs git.** The module proxy comes first. Then the github.com tar.gz, the same through proxy.pazer.ai, the zip, the zip through proxy.pazer.ai, and a shallow git fetch. Depth, and when an archive is refused: docs/GITHUB-FETCH.md.
