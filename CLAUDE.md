...
Three more ubuntu-only suites gate properties nothing else measures. `dats/no-wordspam.dats` caps markdown size, paragraph length and comment runs, and refuses changelog phrasing by name. `dats/goos-readonly.dats` refuses any assignment to `runtime.GOOS` outside `setGOOS`, which the compiler cannot do because it is a variable here. `dats/emitted-binaries.dats` builds a program, a thin build, a test binary and both wasm ports, and reads the first bytes of each: a host ELF, Mach-O or PE leaving a build means a port went missing.

`runtime.GOOS` is a variable and names the host: one APE boots on three kernels. `runtime.GOARCH` stays a constant, because the loader runs the payload built for this machine and third-party code writes `const x = runtime.GOARCH == "amd64"`, which a variable rejects.
