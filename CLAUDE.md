...
- **The go command is the toolchain.** `bin/go` links every tool and starts each as `<self> tool <name>`. Each `pkg/tool/<host>/<name>` is a symlink to it, or a copy on Windows. The main package is `cmd/go/main` and `cmd/go` is `package gocmd`. Each tool is a package whose `Main(args []string) int` is its old `main`. A tool's flags live on its own FlagSet, which `objabi.Enter` installs as `flag.CommandLine`. `go_bootstrap` links no tools. toolchain1 is the same main under `compiler_bootstrap`. The cosmo `os/exec` runs an `ENOEXEC` file opening with `MZqFpD` again under `/bin/sh`. So a cosmo go command runs the test binaries it builds without a wrapper.
