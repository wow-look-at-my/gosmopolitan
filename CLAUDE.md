...
Both name the HOST, read at startup: one APE boots on three kernels, and a payload can run on a machine of another architecture. The cost is that `const x = runtime.GOARCH == "amd64"` is a compile error against a variable. Third-party code writes it (`golang.org/x/crypto/chacha20`), so `crypto/tls`'s bogo suite cannot build its dependency. Only the compiler can serve both readings. It must fold the build value in a constant context while a plain read still loads the variable. It does not yet.
