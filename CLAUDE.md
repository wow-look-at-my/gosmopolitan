...
- **`GOBIN` and `GOTOOLCHAIN` are removed.** Each one let a caller step around what this toolchain guarantees: where a binary lands, and which toolchain builds it. Neither is in `KnownEnv`, so `go env -w` refuses both. `cfg.RemovedEnv` names them, `cfg.UnsetRemovedEnv` drops them from the process environment at startup, and `initEnvCache` drops them from the `go/env` file. `cmd/dist` unsets both too, because it starts the bootstrap go command, which still honors them. Depth: docs/INSTALL.md, "Consumer gotchas".
- **`GOROOT` from the environment is ignored. `-toolexec` is removed.** Each one replaced the tools a build runs. `GOROOT` named another tree's compiler, linker and standard library. `-toolexec` named a program to run in their place. `findGOROOT` derives the tree from this executable alone. It reads neither the environment nor the `go/env` file. `Main` then puts the derived value back into the environment, because the compiler reads `GOROOT` through `internal/buildcfg`. A binary carrying an embedded standard library builds against it whatever the environment says. Depth: docs/EMBEDDED-STD.md.
