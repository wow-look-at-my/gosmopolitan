...
- **The cosmo arm64 image links at 0x1000000000**. The image is not PIE, so the macOS loader maps it with MAP_FIXED after libSystem has run. Apple's malloc places a reservation anywhere between 0x400000000 and past 0xa00000000. An image linked there overwrote live allocations, and every fork child then spun in libobjc's atfork handler. The loader probes the range with a Mach fixed allocation first. It exits by name when any page is in use. `TestFatPayloadsOffLibSystem` pins the address.
...
**`make.bash` installs `std` and the tools with `$GOROOT/bin/go`, not `go_bootstrap`.** bin/go is the only binary the build produces that carries the client. It is therefore the only one that can fetch that compile instead of repeating it. `go_bootstrap` installs `cmd/go` alone, which puts bin/go in place. `checkNotStale` then asserts the two drivers agree. Depth: docs/CI.md.
