...
- **A direct fetch from github.com downloads an archive before it runs git.** The proxy comes first. Then `github.com/<owner>/<repo>/archive/<ref>.tar.gz`, then `.zip`, then a shallow git fetch. Depth, and when an archive is refused: docs/GITHUB-FETCH.md.
