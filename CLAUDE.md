...
- **A direct fetch from github.com downloads an archive before it runs git.** The module proxy comes first. Then the github.com tar.gz, the same through proxy.pazer.ai, the zip, the zip through proxy.pazer.ai, and a shallow git fetch. Refs come over plain HTTP, not `ls-remote`, and no git runs until git is the last option left. Depth: docs/GITHUB-FETCH.md.
