...
- **A direct fetch from github.com downloads an archive before it runs git.** The module proxy comes first. Then the github.com tar.gz, the same through proxy.pazer.ai, the zip, the zip through proxy.pazer.ai, and a shallow git fetch. Depth: docs/GITHUB-FETCH.md.
