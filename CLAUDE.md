...
- **The go command is the toolchain.** `bin/go` links compile, link, asm, cgo, cover, vet, fix and preprofile. It starts each as `<self> tool <name>`. Each `pkg/tool/<host>/<name>` is a symlink to it, or a copy on Windows. A caller that runs a tool by its path still finds it. The main package is `cmd/go/main` and `cmd/go` is `package gocmd`. Each tool is a package whose `Main(args []string) int` is its old `main`. A tool's flags register on a FlagSet of its own, which `objabi.Enter` installs as `flag.CommandLine` when that tool starts. `go_bootstrap` links no tools and runs the pkg/tool links. toolchain1 is the same main under `compiler_bootstrap`, carrying compile, link, asm and cgo. The cosmo `os/exec` starts a pristine APE on a posix host. An `ENOEXEC` file that opens with `MZqFpD` runs again under `/bin/sh`. So a cosmo go command runs the test binaries it builds without a wrapper.
- **A go binary can carry its standard library.** `go tool embedstd -o blob` writes it. `GOCOSMOAPPEND=blob` on a cosmo build appends it past the APE's load span. With no `GOROOT` in the environment that go command builds against the archives in process. It refuses to test or vet std. Depth: docs/EMBEDDED-STD.md.
...
`dats/checks/goos-not-syscall-flags.dats` refuses a `runtime.GOOS` predicate that decides an open flag. `runtime.GOOS` names the HOST, and a syscall flag belongs to the build target, so such a branch must read `internal/goos.IsWindows`. `os/removeall_at.go` shipped that mistake and cost every `t.TempDir` cleanup on the windows leg. The guard skips a file whose build constraint leaves cosmo out, because one that flags code cosmo never compiles trains the reader to skim it.

More suites in `cosmo-checks` gate properties nothing else measures. `dats/checks/unix-tag-agrees.dats` compares `cmd/dist`'s `unixOS` against `internal/syslist.UnixOS`. dist builds against the bootstrap toolchain and cannot import that package, so it copies the list. A GOOS in one copy and not the other makes the two builders disagree about which files a package holds. `dats/checks/no-wordspam.dats` caps markdown size, paragraph length and comment runs, and refuses changelog phrasing by name. `runtime.GOOS` and `runtime.GOARCH` are `readonly var`, so only package runtime may assign them (docs/READONLY-VARS.md). `dats/checks/emitted-binaries.dats` builds a program, a thin build, a test binary and both wasm ports. It reads the first bytes of each. A host ELF, Mach-O or PE leaving a build means a port went missing.

`dats/checks/waiver-expiry.dats` refuses a `continue-on-error` that carries no `waiver-expires:` date. It also refuses one whose date has passed. The NT suite leg holds one until 2026-09-18. It runs and reports in full. It does not hold the branch while the port's own gaps fail packages there.
...
`dats/checks/build-std.dats` is the uprev guardrail: `GOOS=cosmo go build std` for amd64 and arm64, and the x/sys, modernc libc and sqlite consumers against it. The execution suite compiles only what fizzbuzz and runtimeprobe import — 84 of the 358 std packages under cosmo — so an upstream re-partition of a. Run it locally before proposing an uprev — it is what turns a clean merge into a verified one.
...
Consequences to know. **Clone with `--recurse-submodules`**, or `cmd/go` will not build. Every `actions/checkout` in `cosmo-ci.yml` passes `submodules: true` for the same reason. Nobody moves the client by hand. `src/submodulebranch.bash` puts each org submodule on its branch head and stamps the version into `src/cmd/go.mod` and `vendor/modules.txt`, on every build. The committed version is therefore the zero pseudo-version, and `dats/checks/org-unpinned.sh` keeps it that way. **Never run `go mod vendor` here** —. Read `src/README.vendor` before adding any other `src/cmd` dependency: what looks like one import is a whole subtree of somebody else's repository.

**A cmd/go change that needs a new client API rides the client's own branch.** Both repositories carry the branch name, and `branch = .` sends the submodule to it. The merge on each side returns both to their default branches. The gitlink a checkout restores decides nothing. It is the fallback for a build that cannot reach the remote.
