...
- **The cosmo arm64 image is ET_EXEC at 0x800000000**, the address cosmopolitan links its own aarch64 images at. Upstream Go is PIE on darwin, so a normal Go binary there has no fixed address to collide with. This one does, and MAP_FIXED replaces whatever it lands on. The macOS loader now reads the range first: a PROT_NONE reservation is what MAP_FIXED is for, and accessible memory there is fatal and named. `TestFatPayloadsAbovePageZero` pins the payloads above the loader's page zero.
