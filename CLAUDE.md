...
- **The go command is the toolchain.** `bin/go` links compile, link, asm, cgo, cover, vet, fix and preprofile, and starts each as `<self> tool <name>`. Each `pkg/tool/<host>/<name>` is a symlink to it (a copy on Windows), for a caller that runs a tool by its path. The main package is `cmd/go/main`; `cmd/go` is `package gocmd`, and each tool is a package whose `Main(args []string) int` is its old `main`. A tool's flags register on a FlagSet of its own, which `objabi.Enter` installs as `flag.CommandLine` when that tool starts. `go_bootstrap` links no tools and runs the pkg/tool links; toolchain1 is the same main under `compiler_bootstrap`, carrying compile, link, asm and cgo. The cosmo `os/exec` starts a pristine APE on a posix host by re-running an `ENOEXEC` file that opens with `MZqFpD` under `/bin/sh`, so a cosmo go command runs the test binaries it builds without a wrapper.
