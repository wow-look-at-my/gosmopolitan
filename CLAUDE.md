...
# APE is stripped (no DWARF/symtab, cosmocc-style); the amd64 image's
# debug info lands in one sidecar ELF next to it, program.com.dbg. The
# arm64 image gets no sidecar: an ELF for another machine sitting beside
# the APE only invites an attempt to run it.
...
- **The toolchain builds against its own tree.** `findGOROOT` derives it from this executable, and reads neither the environment nor the `go/env` file. Depth: docs/EMBEDDED-STD.md.
...
**A dependency's source is never copied into this tree.** A copy writes somebody else's repository down a second time. `x/sys` and `x/text` were each written down twice over, once per vendor tree. Every vendor path is a submodule now. A submodule names the upstream and one commit of it. A bump is then a pointer move. A diff stays readable. Add a dependency the same way. Run `git submodule add`, then write the require and the `modules.txt` entry. Never run `go mod vendor`.
...
CI builds one fat APE per platform. No GOARCH pin. The output contains cosmo amd64 and cosmo arm64 payloads, stripped by default. apetest's `TestFatSidecarsExist` asserts the `.dbg` sidecar exists on every build. It also asserts that no `.aarch64.elf` does. The artifact ships the bare binaries, so apetest's TestDebugSidecars skips on the test runners). Structural format tests run everywhere. The full execution suite (fizzbuzz + runtimeprobe) runs on all three test runners.
...
**`dist test` never passes `-v`.** It runs each binary into test2json and renders the events. A passing test costs no line. A failing test's output goes through whole. An unfinished binary's buffer is flushed. A progress line each second names the tests that just ended, slowest first, and a table at the end names the slowest of the run. That is where a per-test duration comes from. Never add `-v` back: one subtest per line buries the failures. Depth: docs/CI.md.

The `source-checks` job runs `dats/source`, every guard that reads the tree and needs no toolchain. It waits on no build. A violation is therefore named within the minute. A suite whose commands need a built go goes in `dats/checks` instead.
...
`GOCACHEPROG` is deleted. `GO_BUILDCACHE_CONFIG` configures the tier and an unconfigured CI run fails outright. An entry is bytes under a key of source and compiler. A linked executable is an entry like any other, so a warm `go run` or `go tool` copies the binary out of the cache and starts it. It compiles nothing and links nothing. Upstream also keeps a directory-shaped entry and starts that one in place. This fork does not. So `go tool -n` names a build directory and never the cache. The client is a submodule that tracks this repository's branch, never a pin.

**No dependency source is copied into this tree.** `src/cmd` and `std` both build in vendor mode. Each require needs its packages under `src/cmd/vendor/` or `src/vendor/`. Every one of those paths is a **git submodule**. This repo stores a commit pointer. The source keeps its own history and its own upstream. `.gitmodules` is the list, and `git submodule status` prints each pinned commit.

A third-party submodule pins a release tag, or the commit its version names. An org submodule follows a branch instead, which is what `src/submodulebranch.bash` puts it on.
...
| `src/cmd/vendor/github.com/stretchr/testify` | the assertions every `cmd` test uses |

Consequences to know. **Clone with `--recurse-submodules`**, or `cmd/go` will not build. Every `actions/checkout` in `cosmo-ci.yml` passes `submodules: true` for the same reason. Nobody moves the client by hand. `src/submodulebranch.bash` puts each org submodule on its branch head through `git submodule update --init --remote`. No build stamps a version: the committed version is the placeholder `vN.0.0`, and `dats/source/org-unpinned.sh` keeps it that way. **Never run `go mod vendor` here** —. Read `src/README.vendor` before adding any other `src/cmd` dependency: what looks like one import is a whole subtree of somebody else's repository.
