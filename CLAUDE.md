...
- **The cosmo arm64 image links at 0x400000000**. The image is not PIE, so the macOS loader maps it with MAP_FIXED after libSystem has run. Apple's current malloc zone owns 0x800000000 up. An image linked there overwrote live allocations, and every fork child then spun in libobjc's atfork handler. The loader now reserves the range with a plain mmap first. It exits by name when the kernel answers elsewhere. `TestFatPayloadsOffLibSystem` pins the address.
