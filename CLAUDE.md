...
**An org dependency is never pinned to a version.** Not in `go.mod`. Not in a gitlink. Not in `vendor/modules.txt`. A pin freezes one repository against a commit of another and calls the result reproducible. What it reproduces is a build nobody can fix from here. The cache client's pin is what broke `cmd/go`: `shared.go` named counters the pinned commit does not carry, so `go_bootstrap install cmd/go` failed and no toolchain built. What the two files record for an org module is the placeholder `vN.0.0`, and the go command resolves it in memory to the head of a branch: this repository's own branch when the dependency has one, its default branch otherwise. Every `wow-look-at-my` submodule tracks the same way: `submodulebranch.bash` and `submodulebranch.bat` name the branch to follow and run `git submodule update --init --remote`, which make.bash and make.bat call, and `branch = master` in `.gitmodules` is the branch a submodule without a branch of that name takes. A third-party dependency keeps its version.
