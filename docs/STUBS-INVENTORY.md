...
| `ioctl` | Request numbers encode direction and argument size, so the two systems number even the calls they share differently. Six requests need only that translation: `TIOCGWINSZ`/`TIOCSWINSZ` (struct winsize is four `uint16` on both) and the four job-control requests `syscall.Setctty` and `syscall.Foreground` issue between fork and exec. The termios four need their struct converted too - see below. |

**The termios family** (`TCGETS`, `TCSETS`, `TCSETSW`, `TCSETSF`) landed in the same wave, in `termios_cosmo.go`. It is the one family where nothing lines up: Apple's flag words are 64 bits against Linux's 32, it carries twenty control characters against nineteen and numbers them differently, it keeps the line speeds in their own fields where Linux encodes them inside `c_cflag`, and nearly every flag bit sits somewhere else. Two of the collisions are worse than merely wrong — Linux `IXON` (0x400) is Apple `IXOFF`, and Linux `IUCLC` (0x200) is Apple `IXON` — so a forwarded flag word does not fail, it turns flow control inside out. Three details worth knowing:

- **A set is a read-modify-write.** Apple has settings Linux cannot name (`ALTWERASE`, `NOKERNINFO`, `ONOEOT`, `OXTABS`); writing only what the caller passed would clear them on the get-modify-set every terminal library performs.
- **The Linux-only flags are dropped, not refused**: `IUCLC`, `OLCUC`, `XCASE`, `CMSPAR` and the output-delay fields. Linux itself leaves every one of them to the driver and no terminal driver in use implements any, so failing an otherwise ordinary raw-mode setup over one would be the worse answer. This is the single lossy point, and it is stated rather than silent.
- **An unencodable line speed is refused with EINVAL**, in both directions, rather than reported as some other speed.

The emulation writes exactly the 36 bytes the kernel writes — the `struct termios` `TCGETS` fills, not the larger `termios2` — so a caller that allocated what the kernel fills is not overrun.

**Coverage.** The request tables, the rusage conversion and the whole termios translation — flag maps, `c_cc` indices, baud codes, the round trip, and the Apple-only bits surviving it — are unit-tested in `darwinabi_cosmo_test.go` (ubuntu leg), where they are pure arithmetic that runs on any host. The syscalls run end to end in `testdata/runtimeprobe`'s `flock`, `durable`, `rusage`, `ioctl` and `termios` checks on all three runners. `flock` asserts exclusion between two descriptors and the EWOULDBLOCK errno, not just the return code. `ioctl` asserts on WHICH errno comes back with no terminal attached: ENOTTY is the kernel answering, ENOSYS is the emulation refusing.
...
- The macOS arm64 syscall surface serves everything the `syscall` package exposes and Apple can serve, the termios ioctls included (Sections 6 and 6a). The wording that used to sit here said "complete" while six syscalls were absent, which is how they stayed absent — so before writing that word again, list every `SYS_*` the cosmo `syscall` package names and diff it against the dispatch. What is untested is not the same as what is unbuilt: the termios round trip has never run against a live terminal, because no CI runner has one.
...
5. Exercise the termios conversion against a real pty. The tables are unit-tested and the syscall is proven to reach the kernel (Section 6a), but no CI runner has a terminal, so the round trip has never run against a live driver.
