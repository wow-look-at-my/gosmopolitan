...
| # | Package | Behavior |
|---|---------|----------|
| 0 | every package, on the windows leg | NO cosmo binary built from this tree runs on that host, fizzbuzz included: fat and thin alike die and write nothing. A REGRESSION, not a gap. A matrix bisect over this branch's own commits named it: master, `2b5c9490` and `e2f008f8` all build a fizzbuzz that prints on that runner, and `4e8ff115` - the commit that made GOOS and GOARCH variables - and every commit after it does not. It is a FAULT, not a throw: a leg that captured stderr read exit 139. The boot trace (`-tags cosmontdebug`) shows it reaching the end of `schedinit`: handles, ntResolve, ntBootInit, osinit, randinit, mallocinit, stkobjinit, goargs, goenvs, checkfds, gcinit. So the fault is after that, and the milestones now bracket `procresize`, `minit`, `mstartm0` and `runtime.main`. |
...
| 6 | six packages, on a macOS host | Still red, and each one passes on the ubuntu leg, so every one is about the host rather than the port: `net/http/cgi` (the CGI child gets no PATH and a name with no slash, and the APE loader was handed a path it could not open), `internal/cpu` (fixed on this branch, unconfirmed: `getGodebugEarly` answered nothing for this port, so `cpu.all=off` never reached `cpuinit`), `cmd/addr2line` (a line number one off), `runtime/pprof`'s `TestMemoryProfiler` (a profile entry that does not match), `runtime`'s `TestCrashDumpsAllThreads` and `database/sql`'s `TestConcurrency`. |
