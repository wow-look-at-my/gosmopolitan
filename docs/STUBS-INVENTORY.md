...
| 2 | `os`, on a macOS host | `TestHostname` (uname reports an empty nodename there, so `hostname` falls through to a Linux-only `/proc` path), `TestReadDirFD` (fstatat on `/dev/fd/3` answers EBADF), `TestMkdirStickyUmask` (the mode arrives as 0700), `TestExecutableDeleted` and `TestProgWideChdir`, and the three `Root` chmod tests. |
