...
<<<<<<< HEAD
Os/exec: pipe2 over CreatePipe - blocking, non-pollable on purpose - a posix_spawn-style CreateProcessW path with upstream-ported quoting and env block, and wait4 packing the Linux wait-status protocol: exit = code<<8, NTSTATUS crashes and encoded signal deaths 0xC0DE0000|sig decode as Linux termination signals. exec.LookPath/exec.Command resolve names against the HOST-format PATH (src/os/exec/lp_cosmo.go: runtime host switch. On NT a lp_windows.go port - '.' split, PATHEXT/.exe probing, ErrDot semantics, case-INSENSITIVE PATH/PATHEXT env lookup since NT blocks spell "Path" while cosmo's os.Getenv stays exact-case. Unix hosts keep verbatim lp_unix behavior).
=======
...
The trust store was the same shape of gap one layer up, and is also fixed: every path in crypto/x509's `root_cosmo.go` is a unix. Keyboard chords, window close, LOGOFF/SHUTDOWN, and group-targeted CTRL_C stay documented-not-asserted).
=======
...
**The binaries the NT suite starts are not APEs.** `run.bash` and `run.bat` pin `GOOS` and `GOARCH` to the host values. So `dist test` on each leg tests that host's own port. The windows leg tests `GOOS=windows`. The binaries it builds, copies and starts are native PEs from this fork's linker. `os/exec`'s TestCommand comes from `lp_windows_test.go`. A cosmo build does not compile that file at all. The cosmo port's own coverage is `dats/checks/cosmo-tests.dats`, on the ubuntu leg.

Each property below decides whether NT starts such a PE. A test holds each one.

**A test binary must start under a stripped environment.** The first shard puts every package's tests in one binary. A cgo package rides in with them. The link therefore goes through gcc, and gcc names `libwinpthread-1.dll`. That DLL sits in mingw's own directory. Only PATH reaches it. So `cmd/dist` links that binary against a static mingw runtime. What remains is `kernel32` and the CRT API sets, which the loader finds with no PATH at all. `os/exec`'s TestStrippedPathStartsAChild copies the binary and starts the copy. It uses an inherited PATH, the system directory alone, a dot, and an empty value. It logs every DLL the import table names. The loader reports a missing one only as `0xc0000135`, STATUS_DLL_NOT_FOUND. `dats/test/nt-strippedpath.ps1` holds the other half. A copy of a system binary and a copy of an upstream gofmt each start under those same values. The host therefore stops no program by itself.

**The load config data directory must describe memory the image holds.** Windows 10 and later read that directory before they start an image. They answer one that points outside the image with ERROR_BAD_EXE_FORMAT. Under `-flto`, GNU ld writes it an address of `0xc0000000` and a size of zero. The image ends at `0x2c2000`. So `cmd/link` reads the image after an external link. It zeroes an entry that names memory the image does not hold. A zero entry says the image carries no load config, which Windows accepts. `cmd/cgo/internal/swig`'s TestLinkUnderLTO builds one cgo call twice, plain and under `-flto`. It logs both headers and starts both. Neither swig nor C++ is needed to produce the defect. `dats/test/nt-lto.ps1` keeps the host side measured. gcc links C files under `-flto` here. That result runs.

The probe `cmd/link` already carries for this cannot answer it. The probe links a program of its own. The defect needs the whole link to appear. So the probe reads a sound directory while the real link writes a bad one. Only the image answers for the image.

`os/exec` is upstream on this host, and a diff against go1.27.0 says so. Its startProcess is a passthrough to os.StartProcess wherever the build is not cosmo. Its addCriticalEnv differs only when the parent itself carries no SYSTEMROOT. The runner carries one. Env lookup on NT is case-insensitive. So the all-caps `SYSTEMROOT` key the cmd/go script harness copies does reach the child.

Wine reproduces neither property. It runs a cosmo APE as an NT host. It also runs the windows-targeted os/exec test binary green, TestCommand included. What wine leaves red is its own win32 gap, in Test64BitReturnStdCall and TestEnumWindows.

One fault on the NT leg has no cause yet. A `*testState` pointer read back as `0x1000000000000000` inside `testing.(*T).Parallel`. The fault address came through as `0xffffffffffffffff`. It landed right after TestStackGrowthCallback. That makes the repacked pclntab a candidate, because stack copying reads the funcdata that format moved. It has appeared one time and has not repeated.

A stdlib test branches on `runtime.GOOS`. On cosmo that is a readonly var naming the HOST. An NT runner therefore takes the windows expectations. The package under it compiled `path_unix.go`, whose tag is `unix || (js && wasm) || wasip1`, and cosmo is a unix. The test asks a unix build for windows behavior.
...
<<<<<<< HEAD
The remaining known macOS gaps are AllThreadsSyscall (Linux-only rt-signal machinery, unused by the stdlib on cosmo) and the Intel-mac runtime bring-up below.
...
macOS Intel status: the dd-assimilated Mach-O is structurally correct as of 2026-07-02 (per-PT_LOAD segments with real protections and BSS, __PAGEZERO, host-OS handoff in rcx - verified against the XNU loader's checks by cmd/link unit tests and apetest). The syscall surface closed on 2026-09-02: the metadata table, the XNU carry-flag error convention, Apple-to-Linux errno numbering, the kqueue/kevent netpoller and hw.ncpu.
=======
