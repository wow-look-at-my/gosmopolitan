...
The host refuses nothing. `dats/test/nt-strippedpath.ps1` copies a system binary the way os/exec's TestCommand copies its own, starts it from that copy, and asserts the exit code. It passes on the runner under a dot PATH and under an empty one. So a stripped PATH stops no program there by itself. The binary decides it. The same check copies the upstream go the job installs. That binary carries the same kernel32-only import shape, and the check asks the same question of it.
