...
Symlinks, readlink, lstat, dup for every fd kind and the read-only attribute followed (2026-09-16, `os_cosmo_nt_link.go`): symlinkat over CreateSymbolicLinkW, readlinkat over FSCTL_GET_REPARSE_POINT, lstat and getdents64 reporting a name-surrogate reparse point as a link, dup/dup2/dup3 over DuplicateHandle, unlink clearing FILE_ATTRIBUTE_READONLY before DeleteFileW and taking a directory symlink, and chmod carrying the owner's write bit as that attribute, which stat reports as 0555 against 0755. `os.TempDir` answers in the /c/ spelling Getwd and Executable use, so a path built under it compares equal to one read back.
...
There is still no Intel-mac CI runner, so end-to-end execution there is UNTESTED. Do not claim macOS Intel "works" until the runtime bring-up lands and is verified on real hardware.
