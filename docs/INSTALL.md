...
- **The shipped `go.env` defaults `GOTOOLCHAIN=local`.** Upstream ships `auto`. Under `auto`, a consumer go.mod with a newer `go` or `toolchain` directive silently downloads an official toolchain and loses cosmo. An explicit `GOTOOLCHAIN` env var or `go env -w` still overrides the default. A go.mod genuinely newer than the fork fails loudly.
