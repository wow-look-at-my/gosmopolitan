...
Fork does NOT serialize. The child runs parallel-by-default like any other run, so a forked test's subtests still run at the same time. The rest of the suite keeps running in the parent. A test that needs its own process AND the process to itself calls `t.Serial()` too. Either order works, because the child runs the body from the top.
