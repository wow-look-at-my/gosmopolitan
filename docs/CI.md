...
The fizzbuzz NT boot check starts each binary with `Start-Process` rather than pwsh's native invocation, and reads `$p.ExitCode`. That keeps the check clear of pwsh's own teardown, and the script ends in an explicit exit. A line before and after each run localizes a future hang.
