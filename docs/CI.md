...
**The whole test suite (run.bash).** Every other test step here used to name the tests it wanted, and a test nobody names never runs. That makes green a statement about the list rather than about the tree. It is how the parameter-default tests shipped covered by nothing. `run.bash` is the distribution's own answer: it execs `go tool dist test -rebuild`, which is the single call that runs everything - the stdlib and `cmd` packages, the `test/`. A plain `go test` reaches none of the last two.
