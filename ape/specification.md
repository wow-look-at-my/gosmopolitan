...
Where the decoded header is WRITTEN is the script's own business, and a
parser must not assume it lands in the APE. This implementation copies the
file and writes the header into the copy, so the APE keeps its bytes, its
checksum, and every architecture it carries, and it runs from a read-only
path. A parser reads the header out of the `printf` statement either way.
...
The `skip` and `count` values describe offsets in the APE, whatever file the
`if` and `of` operands name. This implementation names its staged copy there
instead of `$o`, for the reason given under Embedded ELF Header.
