...
tests:
	- desc: no wordspam in the fork's own markdown
	  cmd: dats/checks/no-wordspam.sh *.md docs/*.md dats/*/*.dats dats/*/*.sh
	  exit: 0

	- desc: no wordspam in the cosmo runtime
	  cmd: dats/checks/no-wordspam.sh src/runtime/*cosmo*.go
	  exit: 0

	- desc: no wordspam in the cosmo syscall emulation
	  cmd: dats/checks/no-wordspam.sh src/internal/runtime/syscall/cosmo/*.go src/syscall/*cosmo*.go src/internal/poll/sendfile_shape*.go
	  exit: 0

	- desc: no wordspam in the fork's own std packages
	  cmd: dats/checks/no-wordspam.sh src/internal/ape/*.go src/cmd/internal/cosmoape/*.go src/cmd/internal/objfile/ape.go src/cmd/link/internal/ld/ape*.go
	  exit: 0

	- desc: the scanner refuses a paragraph over the cap
	  cmd: printf 'x %.0s' $(seq 1 200) > "$TMPDIR/spam.md"; dats/checks/no-wordspam.sh "$TMPDIR/spam.md"; test $? -eq 2
	  exit: 0

	- desc: the scanner refuses a comment run over the cap
	  cmd: printf '// x\n%.0s' $(seq 1 20) > "$TMPDIR/spam.go"; dats/checks/no-wordspam.sh "$TMPDIR/spam.go"; test $? -eq 2
	  exit: 0

	- desc: a //sys block is a declaration, not a comment run
	  cmd: printf '//sys\tF%s() (err error)\n' $(seq 1 30) > "$TMPDIR/sys.go"; dats/checks/no-wordspam.sh "$TMPDIR/sys.go"
	  exit: 0
