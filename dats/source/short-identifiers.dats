...
tests:
	...
	- desc: the scanner refuses a one-letter range variable
	  cmd: printf 'package p\n\nfunc F() {\n\tfor i := range xs {\n\t\t_ = i\n\t}\n}\n' > "$TMPDIR/loop.go"; dats/source/short-identifiers.sh "$TMPDIR/loop.go"; test $? -eq 2
	  exit: 0

	- desc: the scanner refuses a two-letter short declaration
	  cmd: printf 'package p\n\nfunc F() {\n\tok := g()\n\t_ = ok\n}\n' > "$TMPDIR/decl.go"; dats/source/short-identifiers.sh "$TMPDIR/decl.go"; test $? -eq 2
	  exit: 0

	- desc: the scanner refuses a one-letter var declaration
	  cmd: printf 'package p\n\nvar b []byte\n' > "$TMPDIR/var.go"; dats/source/short-identifiers.sh "$TMPDIR/var.go"; test $? -eq 2
	  exit: 0

	- desc: the blank identifier is not a name
	  cmd: printf 'package p\n\nfunc F() {\n\tfor _, item := range xs {\n\t\t_ = item\n\t}\n}\n' > "$TMPDIR/blank.go"; dats/source/short-identifiers.sh "$TMPDIR/blank.go"
	  exit: 0

	- desc: a comment naming a short variable is not a declaration
	  cmd: printf 'package p\n\n// i is not declared here.\nfunc F() {}\n' > "$TMPDIR/note.go"; dats/source/short-identifiers.sh "$TMPDIR/note.go"
	  exit: 0
