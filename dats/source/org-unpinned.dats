...
tests:
	- desc: no org dependency in the tree carries a frozen version
	  cmd: dats/source/org-unpinned.sh .gitmodules src/cmd/go.mod src/cmd/go.sum src/cmd/vendor/modules.txt .github/workflows/*.yml .github/actions/*/action.yml
	  exit: 0
	...
	- desc: the guard refuses an org submodule that names no branch
	  cmd: |
		...
		rc=0; dats/source/org-unpinned.sh "$TMPDIR/frozen.gitmodules" || rc=$?
		test "$rc" -eq 2
	  exit: 0
	...
	- desc: the guard accepts an org submodule that names a branch
	  cmd: |
		...
		dats/source/org-unpinned.sh "$TMPDIR/tracked.gitmodules"
	  exit: 0
	...
	- desc: a third-party submodule needs no branch
	  cmd: |
		...
		dats/source/org-unpinned.sh "$TMPDIR/third.gitmodules"
	  exit: 0
	...
	- desc: the guard refuses a dated pseudo-version on an org module
	  cmd: |
		...
		rc=0; dats/source/org-unpinned.sh "$TMPDIR/pinned.mod" || rc=$?
		test "$rc" -eq 2
	  exit: 0
	...
	- desc: the guard refuses a release tag on an org module
	  cmd: |
		...
		rc=0; dats/source/org-unpinned.sh "$TMPDIR/tagged.mod" || rc=$?
		test "$rc" -eq 2
	  exit: 0
	...
	- desc: the guard refuses an org action pinned to a tag
	  cmd: |
		...
		rc=0; dats/source/org-unpinned.sh "$TMPDIR/tag.yml" || rc=$?
		test "$rc" -eq 2
	  exit: 0
	...
	- desc: the guard refuses an org action pinned to a commit
	  cmd: |
		...
		rc=0; dats/source/org-unpinned.sh "$TMPDIR/sha.yml" || rc=$?
		test "$rc" -eq 2
	  exit: 0
	...
	- desc: the guard accepts an org action on a branch
	  cmd: |
		...
		dats/source/org-unpinned.sh "$TMPDIR/branch.yml"
	  exit: 0
