...
// interface.
func bsdInterfaceTable(ifindex int) ([]Interface, error) {
	...
	for _, m := range msgs {
		...
		case *routebsd.InterfaceMessage:
			...
			ift[n].Index = m.Index
			ift[n].Name = m.Name
			ift[n].Flags = bsdLinkFlags(m.Flags)
...
func bsdLinkFlags(rawFlags int) Flags {
	var f Flags
...
// interface.
func bsdInterfaceAddrTable(ifi *Interface) ([]Addr, error) {
	index := 0
	if ifi != nil {
		index = ifi.Index
	}
