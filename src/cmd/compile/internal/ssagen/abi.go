...
func GenWasmExportWrapper(wrapped *ir.Func) {
	...
	// The wrapper generated by cmd/internal/obj/wasm reads the incoming
	// host arguments from at most 16 argument registers. A string
	// parameter counts as two. Generating a wrapper with more parameters
	// would make the assembler index past the argument registers and
	// panic, so report a proper error and emit no body.
	if we := fn.LSym.Func().WasmExport; len(we.Params) > 16 {
		base.ErrorfAt(wrapped.Pos(), 0, "go:wasmexport: too many parameters")
		return
	}
