...
func (pr *pkgReader) objIdxMayFail(idx index, implicits, explicits []*types.Type, shaped bool) (ir.Node, error) {
	...
	// to. Read an object from the export data objReader chose for it, which
	// is the object's own package's when that was imported, rather than from
	// whichever copy names it first: the copies of a package under test,
	// compiled without its _test.go files, lack what those files add.
	if sym.Pkg.Path != pr.PkgPath() {
		if pri, ok := objReader[sym]; ok && pri.pr != pr {
			return pri.pr.objIdxMayFail(pri.idx, nil, explicits, shaped)
		}
	}
...
func (r *reader) pkgInitOrder(target *ir.Package) {
	...
	fn := newPkgInitFunc(pos, "init")
	declared := 1

	// Under -testinit, the variables _test.go files declare are initialized
	// by a function of their own, which runs with the tests.
	var testFn *ir.Func
	var testOrder []ir.Node

	initOrder = initOrder[:0]
	for range cap(initOrder) {
		lhs := make([]ir.Node, r.Len())
		for j := range lhs {
			lhs[j] = r.obj()
		}
		...
		curfn := fn
		if DeferredToTests(pos) {
			if testFn == nil {
				testFn = newPkgInitFunc(base.AutogeneratedPos, "init.test")
				declared++
			}
			curfn = testFn
		}
		...
		if curfn == testFn {
			testOrder = append(testOrder, as)
		} else {
			initOrder = append(initOrder, as)
		}
	}

	fn.Body = initOrder
	if testFn != nil {
		testFn.Body = testOrder
	}
	...
	if testFn != nil {
		staticinit.OutlineMapInits(testFn)
		target.TestInits = append(target.TestInits, testFn)
	}
}

// newPkgInitFunc declares a synthetic function that initializes package-scope
// variables.
func newPkgInitFunc(pos src.XPos, name string) *ir.Func {
	fn := ir.NewFunc(pos, pos, typecheck.Lookup(name), types.NewSignature(nil, nil, nil))
	...
	for i := range nodes {
		...
		case ir.PFUNC:
			...
			if strings.HasPrefix(sym.Name, "init.") {
				if DeferredToTests(name.Pos()) {
					target.TestInits = append(target.TestInits, name.Func)
				} else {
