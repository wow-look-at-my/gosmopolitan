...
func createConservativeVar(fnsym *obj.LSym, fn *ir.Func, n *ir.Name, closureVars map[*ir.Name]int64) *dwarf.Var {
	...
	if n.Esc() == ir.EscHeap && n.Heapaddr != nil && base.Ctxt.Flag_locationlists {
		...
		//
		// A location list also needs location lists to be on: without them
		// there is no loc section for the entry to live in (and no
		// fn.dwarfLocSym). Location lists are off on wasm, which has no
		// DWARF register mapping, and under GOCOSMODEBUG=min.
		debug := fn.DebugInfo.(*ssa.FuncDebug)
