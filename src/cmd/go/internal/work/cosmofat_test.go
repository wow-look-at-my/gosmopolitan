...
// GOCOSMOSHEBANG must reach the linker as a FLAG on every path that writes
// an APE header -- the ordinary link and the fat merge alike -- and only for
// GOOS=cosmo. A flag rather than a linker-side environment read because
// cmd/go hashes the link action from its arguments: an invisible heading
// choice would let a cached binary headed the other way be served.
func TestCosmoShebangLinkArgs(t *testing.T) {
	oldGoos := cfg.Goos
	t.Cleanup(func() { cfg.Goos = oldGoos })
	...
	for _, tt := range []struct {
		...
		{goos: "cosmo", env: "1", want: []string{"-apeshebang"}},
		{goos: "cosmo", env: "on", want: []string{"-apeshebang"}},
		{goos: "cosmo", env: "", want: nil},
		{goos: "cosmo", env: "0", want: nil},
		{goos: "cosmo", env: "yes", want: nil}, // opt-in is exact, like the other knobs' opt-out
		{goos: "linux", env: "1", want: nil},
		{goos: "windows", env: "1", want: nil},
	} {
		cfg.Goos = tt.goos
		t.Setenv("GOCOSMOSHEBANG", tt.env)
		if got := cosmoLinkArgs(); !reflect.DeepEqual(got, tt.want) {
			t.Errorf("GOOS=%s GOCOSMOSHEBANG=%q: cosmoLinkArgs() = %q, want %q",
				tt.goos, tt.env, got, tt.want)
		}
	}

	// The merge builds a fresh header, so it needs the heading told to it
	// again -- a fat build headed MZ despite GOCOSMOSHEBANG=1 is the bug.
	cfg.Goos = "cosmo"
	t.Setenv("GOCOSMOSHEBANG", "1")
	t.Setenv("GOCOSMOSTRIP", "0")
	p := &load.Package{}
	p.Target = "app.com"
	want := []string{"-apefat", "app.com,sib.com", "-o", "app.com", "-apeshebang"}
	if got := cosmoMergeArgs(p, "sib.com"); !reflect.DeepEqual(got, want) {
		t.Errorf("cosmoMergeArgs = %q, want %q", got, want)
	}
}
