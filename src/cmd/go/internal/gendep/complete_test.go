...
// Table.
//
// additions is a file listing and still answers the file. What drops it is
// superseded, below: a tree holding the two declares Table twice and compiles
// as neither, so every consumer of the module gets a package it cannot build.
func TestARedeclaringAdditionIsStillListed(test *testing.T) {
