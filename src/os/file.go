...
//
// It returns $XDG_CACHE_HOME as specified by
// https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html if
// non-empty, else the home directory with /.cache appended. The home directory
// is $HOME, else %USERPROFILE% on Windows and $home on Plan 9.
//
// The layout is the same on every host, which upstream Go does not do: it puts
// the directory under $HOME/Library/Caches on Darwin, %LocalAppData% on Windows
// and $home/lib/cache on Plan 9. One binary here runs on all of those hosts, so
// a per-host layout splits one cache into several.
//
