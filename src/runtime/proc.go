...
func startTheWorldWithSema(now int64, w worldStop) int64 {
	...
	for p1 != nil {
		...
		if p.m != 0 {
			...
			if goarch.IsWasm == 1 && wasmThreadsEnabled && mcount() >= wasmMaxMCount() {
...
func mstartm0() {
	...
	if (iscgo || goos.IsWindows == 1) && !cgoHasExtraM {
		cgoHasExtraM = true
		newextram()
	}
	initsig(false)
...
//go:nowritebarrierrec
func cgoBindM() {
	if goos.IsWindows == 1 || goos.IsPlan9 == 1 {
...
func newm(fn func(), pp *p, id int64) {
	...
	if gp := getg(); gp != nil && gp.m != nil && (gp.m.lockedExt != 0 || gp.m.incgo) && goos.IsPlan9 != 1 && goarch.IsWasm != 1 {
...
func startTemplateThread() {
	if goarch.IsWasm == 1 { // no threads on wasm yet
		return
	}
...
func startm(pp *p, spinning, lockheld bool) {
	...
	nmp := mget()
	if nmp == nil && goarch.IsWasm == 1 && wasmThreadsEnabled && mcount() >= wasmMaxMCount() {
...
top:
	...
	if !haveSysmon {
		if next := wasmForceGCDeadline(); next != 0 && (pollUntil == 0 || next < pollUntil) {
			if pollUntil != 0 || (goos.IsJs != 1 && netpollAnyWaiters()) {
				pollUntil = next
			}
		}
	}
...
func wakeNetPoller(when int64) {
	...
	if sched.lastpoll.Load() == 0 {
		...
		if goos.IsPlan9 != 1 { // Temporary workaround - see issue #42303.
			wakep()
		}
	}
}
...
top:
	...
	if goarch.IsWasm == 1 && wasmThreadsEnabled && mp != &m0 && gp.wasmMainOnly != 0 && gp.lockedm == 0 {
...
func gdestroy(gp *g) {
	...
	if goarch.IsWasm == 1 { // no threads yet on wasm
		gfput(pp, gp)
		return
	}
	...
	if locked {
		...
		// the thread.
		if goos.IsPlan9 != 1 { // See golang.org/issue/22227.
			gogo(&mp.g0.sched)
		} else {
...
func exitsyscallNoP(gp *g) {
	...
	unlock(&sched.lock)
	if pp == nil && goarch.IsWasm == 1 && wasmThreadsEnabled {
...
//go:nosplit
func LockOSThread() {
	if atomic.Load(&newmHandoff.haveTemplateThread) == 0 && goos.IsPlan9 != 1 {
...
func sigprof(pc, sp, lr uintptr, gp *g, mp *m) {
	...
	if goarch.IsMips == 1 || goarch.IsMipsle == 1 || goarch.IsArm == 1 {
		...
		if goarch.IsArm == 1 && goarm < 7 && goos.IsLinux == 1 && pc&0xffff0000 == 0xffff0000 {
...
func checkdead() {
	...
	if (islibrary || isarchive) && goarch.IsWasm != 1 {
		return
	}
	...
	if goos.IsJs == 1 && wasmThreadsEnabled && eventLoopCanWake() {
...
func preemptone(pp *p) bool {
	mp := pp.m.ptr()
	if mp == nil || mp == getg().m {
		if goarch.IsWasm == 1 && mp != nil && mp == getg().m {
...
func mget() *m {
	...
	if goarch.IsWasm == 1 && wasmThreadsEnabled && mp == &m0 {
