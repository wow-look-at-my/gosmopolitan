...
func darwinFcntl(fd, cmd, arg uintptr) (r1, r2, errno uintptr) {
	...
	case linuxF_GETLK, linuxF_SETLK, linuxF_SETLKW:
		return darwinFcntlFlock(fd, cmd, arg)
	default:
		// Owner and lease commands have argument structures with no
		// Apple counterpart; refuse rather than corrupt.
		return ^uintptr(0), 0, darwinENOSYS
	}
...
func darwinFcntlFlock(fd, cmd, arg uintptr) (r1, r2, errno uintptr) {
	...
	r1, r2, errno = darwinCallVariadic1(darwinFns.Fcntl, fd, acmd, uintptr(unsafe.Pointer(&af)))
	if errno != 0 || cmd != linuxF_GETLK {
		return r1, r2, errno
	}

	// F_GETLK answers in the caller's struct: the holder of the conflicting
	// lock, or F_UNLCK when there is none.
	if !flockFromApple(lk, &af) {
		return ^uintptr(0), 0, darwinEINVAL
	}
	return r1, r2, errno
}
