...
(() => {
	...
	if (!globalThis.fs) {
		...
		globalThis.fs = {
			...
			writeSync(fd, buf) {
				let outputBuf = (outputBufs.get(fd) ?? "") + decoder.decode(buf);
			...
			this.importObject = {
				...
				gojs: {
					...
					"syscall/js.copyToGo": (sp) => {
						...
						new typedArrays[kind](this._inst.exports.mem.buffer, dst, toCopy.length).set(toCopy);
						setInt64(sp + 40, toCopy.length);
						this.mem.setUint8(sp + 48, 1);
					},
					...
					"syscall/js.copyToJS": (sp) => {
						...
						dst.set(new typedArrays[kind](this._inst.exports.mem.buffer, src, n));
						setInt64(sp + 40, n);
						this.mem.setUint8(sp + 48, 1);
					},
